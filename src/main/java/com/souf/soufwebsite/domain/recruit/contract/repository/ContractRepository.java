...
public interface ContractRepository extends JpaRepository<Contract, Long> {
    ...
    @Query("select c from Contract as c where c.beneficiary = :beneficiary and c.chatRoom = :chatRoom")
    Optional<Contract> findByBeneficiaryAndChatRoomAndContractStatus_PendingCounterpart(
            @Param(value = "beneficiary") Member beneficiary, @Param(value = "chatRoom") ChatRoom chatRoom);
}
